...
1. Tipos de métodos ou algoritmos (somente-presença, presença-background, presença-ausência)
1. Ausências, pseudo-ausências e background
1. Somente-presença (BIOCLIM, DOMAIN e Mahalanobis)
1. Presença-(pseudo)ausência (GLM, GAM, Random Forests e SVM)
1. Presença-background (MaxEnt)
1. Outros métodos ou algorítmos
...
1. Tipos de partição dos dados (cross-validation, bootstrap, jackknife)
...
background-image: url(img/sdm_limite01.png), url(img/sdm_limite02.png)
...
| BIO14    | Precipitação | Precip. do mês mais seco    |
...
- Encontra parâmetros que **aproximam a forma da distribução** (maximiza a entropia) dos valores ambientais do background e em função dos valores ambientais das presenças
...
# 5.4 Outros métodos ou algorítmos
...
- Dados usados para ajuste do modelo

### Teste (flexa inferior)
- Dados usados para validação ou teste do modelo
...
- **Interna **  
Partição ou reamostragem dos mesmos dados de <br> treino e teste
...
- **Split sample (*chamado de bootstrap)**  
Divide aleatoriamente as ocorrências em dois conjuntos, <br> geralmente 70% para treino e 30% para teste e <br> faz repetições aleatporias desses conjuntos
...
Uma ocorrência para teste e o restante para treino, <br> alteranto até todas as ocorrências irem para teste

- **Bootstrap**  
Reamostragem para medir incertezas dos ajustes
...
background-image: url(img/sdm_avaliacao_calibracao.png), url(img/sdm_avaliacao_observados_preditos.png)
...
background-image: url(img/sdm_avaliacao_calibracao.png), url(img/sdm_avaliacao_observados_preditos_teste.png)
background-size: 500px, 430px
background-position: 10% 62%, 90% 58%
...
- Sensibilidade (ou proporção de positivos verdadeiros - PPV) 
- Especificidade (ou proporção de negativos verdadeiros - PNV)
